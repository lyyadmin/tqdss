;(function(window, factory) {
    ...
    function jeDatePick(elem, options){
        var config = {
            ...
            customdate:[],                              //自定义每个月随意抽取天数
            isDYAjax:false,                             //为机务初期日期的特定处理
    ...
    jet.extend(jeDatePick.prototype,{
        ...
        methodEventBind : function() {
            ...
            jet.extend(that,{
                ...
                //确认按钮函数
                sureBtn:function () {
                    var newArr = that.selectValue.length > 1 ? [{},{}]: [{}], dateVal = "", tmval = that.selectTime,isajax=opts.isDYAjax;
                    ...
                    opts.donefun && opts.donefun.call(that,{elem:that.valCell,val:dateVal,date:newArr});
                    var tm = $("#ondutytime").val();
                   //此处调用请求ajax方法，待优化    isDYAjax
                    if(isajax){
                        $.ajax({
                            url:path+"/checkout/checkOutBuffetQuery/getTuiQinDate.do",
                            type:"post",
                            dataType:"json",
                            data:{"ondutytime":tm,"driver_code":driver_code},
                            success:function(data){
                                if(data.length>0){
                                    var tuiqinTime = data[0].tuiqin_time;
                                    $("#offdutytime").val(tuiqinTime);
                                }else{
                                    $("#offdutytime").val("");
                                }
                            }
        ...
        eachDays  : function (yd,md,ds,idx) {
            var that = this, count = 0, daysArr = [],opts = that.$opts, multiPane = jet.isBool(opts.multiPane),
                ...
                lang = opts.language, endval = opts.valiDate||[],customdateval=opts.customdate||[],
            ...
            //循环调用的天数传入进来，判断是否可用，也就是限制天数选择
            //date 天  cls  ym年月
            var regExpDate = function (date,cls,ym) {
                var inArray = function (search,array){
                    for(var i in array) if(array[i]==search) return true;
                    return false;
                };
                //endval是valiDate传入进来的数据  与生成日期进行比较，然后控制显示天数  customdateval
                ...
                }else if(customdateval.length>0){
                    //遍历传入的集合参数
                    var days;//获取天数
                    for(i in customdateval){
                        if(customdateval[i].ym==ym){
                            days = customdateval[i].days;
                            break;
                        }
                    }
                    var is_days=false;
            ...
            for (var p = prevDaysNum - firstWeek + 1; p <= prevDaysNum; p++, count++) {
                ...
                pcls = regExpDate(p,pcls,prevM.y+"-"+prevM.m);
            ...
            for(var n = 1, nlen = 42 - count; n <= nlen; n++){
                ...
                ncls = regExpDate(n,ncls,nextM.y+"-"+nextM.m);
