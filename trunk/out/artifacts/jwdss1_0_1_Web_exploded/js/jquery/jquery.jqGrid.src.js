...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if(ts.p.hidegrid===false){
			if(ts.p.closebtn===true){
				var closebtn = '<button type="button" style="color:#FFF;margin-right: 5px;" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>';
				$(grid.cDiv).append(closebtn);
			}
		}
		if(ts.p.titleInfo===true){
			var info = $("<div id='"+ts.p.id+"-info'>头部信息</div>");
			grid.hInfo = document.createElement("div");
			$(grid.hInfo)
				.css({ width: "100%",height:'auto'})
				.addClass("ui-widget-content ui-jqgrid-hinfo")
				.append(info);
			$(grid.hDiv).before(grid.hInfo);
		}
		if(ts.p.toppager) {
